...
Public Partial Class frmMain
	...
	Private Sub RefreshLoadedPlayer()
		...
		For Each label As var In Me.m_InventoryLabels
	...
	Private Sub tscboQuickSelect_SelectedIndexChanged(sender As Object, e As EventArgs)
		...
		Dim file = selectedItem.ToString().Split(New () {" -- "}, StringSplitOptions.RemoveEmptyEntries)(1)
		...
		Me.Player = Terraria.Instance.LoadProfile(String.Format("{0}\{1}", Terraria.ProfilePath, file))
		...
		Me.Player.File = String.Format("{0}\{1}", Terraria.ProfilePath, file)
		Me.playerBindingSource.DataSource = Me.Player
		Me.modelViewer.UpdateTextures()
	...
	Private Sub checkForUpdatesToolStripMenuItem_Click(sender As Object, e As EventArgs)
		Try
			...
			Dim latest = client.DownloadString("https://raw.github.com/atom0s/TSGE/master/latest.txt")
			If String.IsNullOrEmpty(latest) Then
				Throw New Exception("No update found!")
			End If

			' Attempt to get the update parts..
			Dim parts = latest.Split(Environment.NewLine.ToCharArray(), StringSplitOptions.RemoveEmptyEntries)
			If parts.Length <> 2 Then
				Throw New Exception("No update found!")
			End If

			' See if we have the latest version..
			If Assembly.GetExecutingAssembly().GetName().Version < New Version(parts(0)) Then
				' Update found..
				Dim ret = MessageBox.Show("Update available!" & vbCr & vbLf & "Version: " & parts(0), "Update available!", MessageBoxButtons.YesNo, MessageBoxIcon.Information)
